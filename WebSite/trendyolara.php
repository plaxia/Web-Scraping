...
if($_GET){
	$kelime=$_GET['giris'];
	if(!$kelime){
		$sql="SELECT * FROM `trendyol` WHERE `marka_model` 
";
	}
	else{
		$sql="SELECT * FROM `trendyol` WHERE `marka_model` LIKE '$kelime'
";
	}
}
 	...
 	<table id="customers " class="table table-hover table-dark">
 		...
 		<tr>
 			...
 			<div class="btn-group">
 				...
 				<a href="<?=$satir['link']?>">
 									<input type="image" src="img/trendyol.png" value="submit" width="70px" >
 				</a>
