...
if(isset($_GET['sil'])){
    $sqlsil="DELETE FROM `vatan` WHERE `vatan`.`fiyat` = ?";
...
if($_GET){
	$kelime=$_GET['giris'];
	if(!$kelime){
		$sql="SELECT * FROM `vatan` WHERE `marka_model` 
";
	}
	else{
		$sql="SELECT * FROM `vatan` WHERE `marka_model` LIKE '$kelime'
";
	}
}
   ...
   <div class="search">
 	...
 	<table id="customers " class="table table-hover table-dark">
 		...
 		<tr>
 			...
 			<div class="btn-group">
 				<a href="vatanguncelle.php?id=<?=$satir['id']?>" class="btn btn-success">Güncelle</a>
 				
 				<a href="?sil=<?=$satir['fiyat']?>" onclick="return confirm('Silinsin mi?')" class="btn btn-danger">Kaldır</a>
 				
 				<a href="<?=$satir['link']?>">
 									<input type="image" src="img/vatan.png" value="submit" width="70px" >
 				</a>
