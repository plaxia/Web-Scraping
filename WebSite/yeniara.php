...
if(isset($_GET['sil'])){
    $sqlsil="DELETE FROM `yeni` WHERE `yeni`.`fiyat` = ?";
    $sorgusil=$baglan->prepare($sqlsil);
    $sorgusil->execute([
        $_GET['sil']
    ]);

    header('Location:yeniara.php');
...
";
		...
		$sql="SELECT * FROM `yeni` WHERE `site` LIKE '$kelime' or
		`marka_model` LIKE '$kelime'";
	}
 	...
 	<table id="customers " class="table table-hover table-dark">
 		...
 		<tr>
			...
			<td><?=$satir['site']?>
 			...
 			<div class="btn-group">
 				<a href="yeniguncelle.php?id=<?=$satir['id']?>" class="btn btn-success">Güncelle</a>
